...
RUN mkdir -p /var/trac/main

RUN trac-admin /var/trac/main initenv project sqlite:db/trac.db
RUN echo "#!/bin/bash" > /usr/local/bin/start_trac.sh
RUN echo '/usr/local/bin/tracd --port 8000 /var/trac/main' >> /usr/local/bin/start_trac.sh
RUN chmod +x /usr/local/bin/start_trac.sh

# wysiwyg
RUN easy_install https://trac-hacks.org/svn/tracwysiwygplugin/0.12
RUN trac-admin /var/trac/main config set components tracwysiwyg.* enabled

# pdf preview
RUN easy_install https://trac-hacks.org/svn/pdfpreviewplugin/1.0/
RUN trac-admin /var/trac/main config set components tracpdfpreview.pdfpreview.pdfrenderer enabled

# enable git repositories
RUN trac-admin /var/trac/main config set components tracopt.versioncontrol.git.* enabled

# githubsync
RUN pip install GitHubSyncPlugin
RUN trac-admin /var/trac/main config set components githubsync.api.* enabled

# include macro
RUN easy_install https://trac-hacks.org/svn/includemacro/trunk/
RUN trac-admin /var/trac/main config set components includemacro.* enabled

# wikiextras
RUN easy_install https://trac-hacks.org/svn/wikiextrasplugin/trunk
RUN trac-admin /var/trac/main config set components tracwikiextras.* enabled

# toc macro
RUN easy_install https://trac-hacks.org/svn/tocmacro/0.11
RUN trac-admin /var/trac/main config set components tractoc.* enabled

# wikiprint
RUN easy_install https://trac-hacks.org/svn/tracwikiprintplugin/1.0
RUN trac-admin /var/trac/main config set components wikiprint.* enabled

# fullblog
RUN easy_install https://trac-hacks.org/svn/fullblogplugin
RUN trac-admin /var/trac/main config set components tracfullblog.* enabled
RUN trac-admin /var/trac/main upgrade

# tractheme engine
RUN pip install TracThemeEngine
RUN trac-admin /var/trac/main config set components themeengine.* enabled

# tracpath theme
RUN easy_install https://trac-hacks.org/svn/tracpaththeme/0.12
RUN trac-admin /var/trac/main config set components tracpaththeme.* enabled

# codeexample macro
RUN easy_install https://trac-hacks.org/svn/codeexamplemacro
RUN trac-admin /var/trac/main config set components codeexample.code_example_processor.* enabled

# graphviz
RUN easy_install https://trac-hacks.org/svn/graphvizplugin/trunk
RUN trac-admin /var/trac/main config set components graphviz.graphviz.graphviz enabled

# accountmanager
RUN easy_install https://trac-hacks.org/svn/accountmanagerplugin/tags/acct_mgr-0.4.4
RUN trac-admin /var/trac/main config set components acct_mgr.* enabled
RUN trac-admin /var/trac/main config set components trac.web.auth.LoginModule disable
RUN echo '' >> /var/trac/main/conf/trac.ini
RUN echo '[account-manager]' >> /var/trac/main/conf/trac.ini
RUN echo 'password_store = HtPasswdStore' >> /var/trac/main/conf/trac.ini
RUN echo 'htpasswd_hash_type =' >> /var/trac/main/conf/trac.ini
RUN echo 'htpasswd_file = /var/trac/main/.htpasswd' >> /var/trac/main/conf/trac.ini
...
RUN trac-admin /var/trac/main config set components plantuml.* enabled
RUN echo '' >> /var/trac/main/conf/trac.ini
RUN echo '[plantuml]' >> /var/trac/main/conf/trac.ini
RUN echo 'plantuml_jar = /opt/plantuml.jar' >> /var/trac/main/conf/trac.ini

# sensitive tickets
RUN easy_install https://trac-hacks.org/svn/sensitiveticketsplugin
RUN trac-admin /var/trac/main config set components sensitivetickets.sensitivetickets.sensitiveticketspolicy enabled
RUN sed -i 's/permission_policies =/permission_policies = SensitiveTicketsPolicy,/g' /var/trac/main/conf/trac.ini
RUN trac-admin /var/trac/main upgrade

# multiproject
RUN easy_install https://trac-hacks.org/svn/simplemultiprojectplugin
RUN trac-admin /var/trac/main config set components simplemultiproject.* enabled
RUN sed -i 's/permission_policies =/permission_policies = ProjectTicketsPolicy,/g' /var/trac/main/conf/trac.ini
RUN sed -i 's/\[ticket-custom\]/\[ticket-custom\]\nproject = select\nproject.label = Project\nproject.value =/g' /var/trac/main/conf/trac.ini
RUN trac-admin /var/trac/main upgrade
...
# permissions
RUN trac-admin /var/trac/main permission add admin TRAC_ADMIN
ADD set_password.py /tmp
RUN python /tmp/set_password.py /var/trac/main admin passw0rd

# cache directory
RUN mkdir -p /var/trac/main/files/cache

# Increase maximum size values
RUN sed -i 's/262144/4000000/g' /var/trac/main/conf/trac.ini

# Set minimum username length to 3
RUN sed -i 's/\[account-manager\]/\[account-manager\]\nusername_regexp = (?i)^\[A-Z0-9.\\-_\]{3,}$/g' /var/trac/main/conf/trac.ini

# logo
ADD logo.png /var/trac/main/htdocs/your_project_logo.png

# Move trac.ini out of the volume
RUN mkdir /etc/trac
RUN mv /var/trac/main/conf/trac.ini /etc/trac
RUN chmod 700 /etc/trac/trac.ini
RUN ln -s /etc/trac/trac.ini /var/trac/main/conf
